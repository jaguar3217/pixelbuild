...
TileSelector::TileSelector(wxFrame* Parent, GamePanel* Panel) :
	wxPanel(Parent, wxID_ANY, wxDefaultPosition, wxSize(400, 150))
{
	...
	for (int i = 0; i < bmp.GetWidth() / 32; i++)
	{
		...
		wxBitmapButton* btn = new wxBitmapButton(this, 10000 + i, newBitmap,
...
void TileSelector::OnClick(wxCommandEvent& evt)
{
	...
	wxButton* btn = wxDynamicCast(evt.GetEventObject(), wxButton);
	long no = btn->GetId() - 10000;
	m_panel->SetTileToPaint(no);
}